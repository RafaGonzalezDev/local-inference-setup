...
rem Single profile: text-only (no vision) and the full validated long context.
...
rem Vision: disabled. No --mmproj is passed, so llama.cpp does not load the
rem vision projector and the model behaves as text-only. The image token cap
rem (--image-min/--image-max) is therefore not applicable.
set "VISION_ARGS="
